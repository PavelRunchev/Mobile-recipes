...
function RecipeModel({ recipe, scrollPosition, favoriteHandler }) {
    ...
    return (
        <Card outline className='card-recipe-container'>
            ...
            <CardContent className='card-recipe-content-container'>
                ...
                <div>
                    <FaUtensils size={18} color='teal'/> <span className='recipe-model-servings'>{recipe.servings}</span>
                </div>
            </CardContent>

            <CardFooter className='recipe-footer'>
                <div onTouchStart={(e) => favHandler(e)} >
                    {favorite 
                        ? <div><BsHeartFill color='teal' size={24} className='recipe-model-icon-favorite'/></div>  
                        : <div><FaRegHeart color='teal' size={24} className='recipe-model-icon-favorite'/></div>
                    }
                </div>
                <Link href={`/recipe/recipe-details/:${recipe.id}/`} className='recipe-link recipe-model-read-more'>Read more</Link>
                <div>
                    <BsStarHalf color='teal' size={24}/> <span className='recipe-model-rating'>{recipe.rating}</span>
                </div>
            </CardFooter>
        </Card>
    )
